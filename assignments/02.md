...
- [Submit your assignment here](https://github.com/product-jam-2026/course/issues/3)
...
In this part you will implement a page displaying data from the Met Museum of Art API to
show a list of cool art.

- Create new page at the route `/art`. Copy `demos/00_blank` to use as a
  starting point.

- Read the [API Docs](https://metmuseum.github.io/) and see the endpoints available to you

- Implement a data fetching function with the signature `async function getArtData(departmentId: string, count: number)`, as seen in class.
  The function fetching an array of art objects from the Met API.
  The array should be of length `count` and with art objects only from the given `departmentId`.
  The objects should be chosen at random from all the art in the department.
  The function can make as many HTTP requests as needed, and it should return a Javascript object (i.e. parsed JSON), structured as you wish.
  
- Handle
    [HTTP errors](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status) by displaying a helpful error message to the user.

- Display the title of the chosen art department in the page, the title should be taken from the API.

- Render a react component for each art item in the array. Make sure to include
  the `title`, `artistDisplayName`, and the image, as well as (at least) 3 other properties returned from the API.
  Style using CSS modules as you wish.
